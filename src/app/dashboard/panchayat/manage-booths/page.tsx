...
export default function ManageBoothsPage() {
  ...
  const fetchBooths = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/dashboard/panchayat/booths');
      if (res.ok) {
          ...
          {filtered.map((w) => (
            <div key={w.id} className="group p-8 rounded-[2.5rem] bg-white border border-gray-100 hover:border-emerald-600/20 shadow-sm transition-all space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full uppercase tracking-widest">{w._count.users} Admins</span>
              </div>
