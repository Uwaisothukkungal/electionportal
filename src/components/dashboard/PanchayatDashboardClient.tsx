...
export function PanchayatDashboardClient({
  totalPredicted,
  totalPolled,
  totalElectorate,
  panchayatName,
  ...
  const filteredBooths = initialBooths.filter(w => {
    ...
    return w.number.toString().includes(term) || (w.name?.toLowerCase().includes(term));
  });
  ...
  const reportData = {
    title: `Election Performance Report - ${panchayatName} Panchayat`,
    partyName: partyName,
    metrics: {
    ...
    tableHeader: ['Booth Number/Name', 'Predicted', 'Polled', 'Percentage'] as [string, string, string, string],
    tableData: filteredBooths.map(w => ({
      name: `Booth ${w.number} ${w.name ? `(${w.name})` : ''}`,
      total: w.predicted,
      polled: w.polled,
    ...
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-black text-gray-900 tracking-tight uppercase">Panchayat Dashboard</h1>
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">Local Performance Summary for {partyName}.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1">
           <PerformanceAnalytics 
             ...
             label="Party vs Local Electorate" 
           />
        </div>
      </div>
      ...
      <div className="pt-10 border-t border-gray-100 space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
          <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest">Booth Performance</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredBooths.map((w) => (
            <Link 
              key={w.id} 
              href={`/dashboard/panchayat/booth/${w.id}`}
            ...
            >
              <div className="space-y-1">
                <h3 className="text-xl font-black text-gray-900 uppercase tracking-tight">Booth {w.number} {w.name && `· ${w.name}`}</h3>
